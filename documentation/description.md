...
5. Календарь.
6. Списки ожидания - задачи, выполнения которых зависит от внешних факторов.
...
- _Доска_ - это хранилице статус-таблиц, в рамках темы которого создаются задачи и проекты.
  > Атрибуты: название.

_Проекты_ состоят из _Задач_ (один из многих).
_Статус-таблицы_ состоят из _Задач_ и _Проектов_ (один из многих).
_Доски_ состоят из _Статус-таблиц_ (один из многих).
_Доски_ принадлежат _Пользователям_ (многие ко одному).
...
- название: тип данных - varchar(128), не должен быть пустым;
...
- срок: тип данных - date.
  ...
  > !ПРИМЕЧАНИЕ
  > Узнать дату конца или начала выполнения проекта можно через первую созданную или последную выполненную задачу, принадлежащую проекту.
...
В досках по-умолчанию присутвуют 6 статус-таблиц: корзина, проекты, отложенные задачи, текущие задачи, ожидание, выполнено. При этом возможно добавить дополнительные _статус-доски_ под нужды пользователя. Дополнительные _статус-таблицы_ воспринимаются в качестве стадий выполнения задач, они не могут служить изначальным статусом.
Созданные _задачи_ изначально попадают в _корзину_. Присвоить _статус_ возможно либо при создании, либо после создания.
Присваивание _статуса_ производится последством вопросов и соответсвующих кнопок согласно [Описанию метологии GTD](#описание-метологии-gtd):
...
```
Проекты не могут находится в _статус-таблицах_ корзина, отложенные задачи, текущие задачи и ожидание. Следовательно, все пункты, содержащиеся в перечисленных досках являются задачами.

Существуют расширенные виды статус-таблицы, задач и проектов, в которых отображается более подробная информация.
...
- кнопка создания в корзину;
- кнопка создания;
- кнопка распределения в зависимости от вида задачи (проектная или самостоятельная).
...
Перенос задач по статус-таблицам производится посредством нажатия кнопки (->) или перетягивания ее курсором.
...
Проекты могут содержаться только в двух статус-таблицах: _Проекты_ и _Выполнено_, так как их статус выполнения проявляется в задачах, из который они состоят.
...
Для изменения порядка пользовательских статус-таблиц необходимо изменять таблицу.
...
Поле _номер телефона_ не является обязательным, но в случае подтверждения Telegram бот будет оповещать пользователя в личные сообщения.
